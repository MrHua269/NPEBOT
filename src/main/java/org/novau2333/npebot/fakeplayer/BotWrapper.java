...
import org.novau2333.npebot.ProxyPool;
...
public class BotWrapper {
    ...
    public static void runNewBot(InetSocketAddress address, String username, String accessToken,boolean enableProxty){
        ...
        new Thread(() -> {
            while (true){
                if (enableProxty && ProxyPool.proxy.size() >= 0) {
                    ProxyPool.proxy.forEach(p -> {
                        try {
                            String[] _p = p.split(":");
                            ...
                            runBot(session,address, username, accessToken, proxy,enableProxty,p1);
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
                    });
                }else{
                    runBot(session,address, username, accessToken,null,enableProxty,null);
                }
            }
        },"MCChatBot-Thread").start();
    }

    public static void runBot(AtomicReference<TcpClientSession> session,InetSocketAddress address, String username, String accessToken,ProxyInfo proxy,boolean enableProxty,Proxy p1){
        try{
